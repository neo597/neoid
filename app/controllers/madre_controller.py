...
class MadreController:
    @staticmethod
    def list_madres():
        madres_ref = get_firestore().collection("madres").stream()
        return [madre.to_dict() for madre in madres_ref]

    @staticmethod
    def get_madre(madre_id: str):
        madre_ref = get_firestore().collection("madres").document(madre_id)
        madre = madre_ref.get()
        return madre.to_dict() if madre.exists else {"error": "Madre no encontrada"}

    @staticmethod
    def search_madre_by_document(documento: str):
        madres_ref = get_firestore().collection("madres").where("numero_documento", "==", documento).stream()
        return [madre.to_dict() for madre in madres_ref]

    @staticmethod
    def search_madres_by_creation(fecha: str):
        madres_ref = get_firestore().collection("madres").where("fecha", "==", fecha).stream()
        return [madre.to_dict() for madre in madres_ref]

    @staticmethod
    def create_madre(madre: MadreBase):
        madre_dict = madre.dict()
        madre_ref = get_firestore().collection("madres").document(madre.id_madre)
